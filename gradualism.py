...
def main():
    ...
    for section in sections:
        ...
        for step in sections[section]:
            ...
            shell_command = re.search('`\$ ?([^`]+)`', step)
            code_block = re.search('```(\w+)([\s\S]+?)```', step)
            step_number = re.match(r"(\d+\.)", step)[1]
            if var_matches:
                ...
                ans = input(f"{step_number} Run {code_lines} lines of {language} [y/n]? ")
                if ans.startswith("y"): 
                ...
                for sv in shell_vars:
                    if os.getenv(sv):
                        cmd = cmd.replace('$' + sv, os.getenv(sv))

                shell = os.getenv("SHELL")
                if not shell:
                    shell = "your shell"
                ans = input(f"{step_number} {cmd}\n\n    Run this in {shell} [y/n]? ")
                if ans.startswith("y"): 
                    try:
                ...
                ans = input('\n' + step + "\n\n    y => continue  |  s => skip  |  b => go back 1")
                if not ans.startswith("y"): 
                    was_task_completed = False
                elif ans.startswith("s"): 
                    print("Skipping this step...")
            else:
                ans = input('\n' + step + "\n\n    y => continue  |  b => go back 1")
                if not ans.startswith("y"): 
                    was_task_completed = False
        ...
        for key in user_vars:
            if "personal" not in key:
                frontmatter += f"\n{key}: {user_vars[key]}" 
        frontmatter += "\n---\n"
        sanitized_section = re.sub(r"[^A-Za-z0-9-_]", "", sect_date.replace(' ', '_'))
